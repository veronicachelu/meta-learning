...
def set_image_bandit_11_arms(values, target_arm, selection, trial):
    ...
    for i in range(11):
        if i == target_arm:
            new_values[0] += values[i]
        elif i == 10:
            new_values[1] += values[i]
        else:
            new_values[2] += values[i]

    bandit_image[115:115 + floor(new_values[0] * 10 / 5 * 2.5), (20 + delta * 1):(75 + delta * 1), :] = [0, 255.0, 0]
    bandit_image[115:115 + floor(new_values[1] * 10 / 5 * 2.5), (20 + delta * 2):(75 + delta * 2), :] = [0, 255.0, 0]
    bandit_image[115:115 + floor(new_values[2] * 10 / 5 * 2.5), (20 + delta * 0):(75 + delta * 0), :] = [0, 255.0, 0]

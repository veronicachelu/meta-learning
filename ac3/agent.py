    ...
    while self.network.T < config.EPISODES:
      ...
      while not (done or ((self.time_step - self.time_step_start) == config.MAX_TIME_STEPS)):
        ...
        if self.time_step_thread % config.PRINT_STATISTICS_EVERY_X_STEPS == 0:
          print "Thead step: ", self.time_step_thread, "Policy probability max, ", np.max(policy_output_values), \
      ...
      if done:
        ...
        print "THREAD:", self.thread_id, "/ TIME", self.network.T, "/ TIMESTEP", self.time_step_thread, "/ REWARD", ep_reward, \

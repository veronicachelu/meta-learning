...
class ConvNetwork():
    def __init__(self, scope, trainer, global_step=None):
        with tf.variable_scope(scope):
            self.inputs = tf.placeholder(shape=[None, FLAGS.height, FLAGS.width, FLAGS.game_channels],
                                         dtype=tf.float32, name="Inputs")

            self.conv_1 = tf.contrib.layers.conv2d(
                self.inputs, 16, 8, 4, activation_fn=tf.nn.elu, scope="conv1")
            self.conv_2 = tf.contrib.layers.conv2d(
                self.conv_1, 32, 4, 2, activation_fn=tf.nn.elu, scope="conv2")
            ...
            self.fc = tf.contrib.layers.fully_connected(tf.contrib.layers.flatten(self.conv_2), 256)
            ...
            if FLAGS.meta:
                ...
                self.prev_rewards_onehot = tf.one_hot(self.prev_rewards, self.action_size + 1, dtype=tf.float32,
